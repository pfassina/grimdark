...
from ..core.events import TurnEnded, TurnStarted, UnitDefeated, UnitMoved, UnitSpawned
...
class Game:
    ...
    def handle_menu_input(self, event: InputEvent) -> None:
        """Handle input events when a menu is open.
        ...
        Args:
            event: The input event to handle (will be used when implemented)
        """
        _ = event
            ...
            if self.state.is_in_movement_range(cursor_x, cursor_y):
                if self.state.selected_unit_id:
                    ...
                    if unit:
                        ...
                        to_pos = (cursor_x, cursor_y)
                        if game_map.move_unit(unit.unit_id, cursor_x, cursor_y):
                            # Emit unit moved event
                            self._emit_unit_moved(
                                unit.name, unit.team, from_pos, to_pos
                            )
                        unit.has_moved = True
    ...
    def _position_cursor_on_next_player_unit(self) -> None:
        ...
        current_unit_at_cursor = game_map.get_unit_at(
    ...
    def _execute_attack(self) -> None:
        ...
        aoe_tiles = game_map.calculate_aoe_tiles(
            (cursor_x, cursor_y), attacker.combat.aoe_pattern
        )
        ...
        for tile_x, tile_y in aoe_tiles:
            ...
            if (
                target and target.unit_id != attacker.unit_id
            ):  # Can hit any unit except the attacker
    ...
    def _perform_attack_damage(self, attacker, targets_hit) -> None:
        ...
        for target in targets_hit:
            ...
            if target.hp_current <= 0:
                ...
                self._emit_unit_defeated(target.name, target.team, target_pos)
        ...
        if len(targets_hit) > 1:
            print(
                f"{attacker.name}'s {attacker.combat.aoe_pattern} attack hits {len(targets_hit)} targets!"
            )
            ...
            if self.state.selected_unit_id:
                ...
                if unit and not unit.has_moved:
                    ...
                    if (
                        ...
                        from_pos = (unit.x, unit.y)
                        to_pos = (
                            self.state.original_unit_x,
                            self.state.original_unit_y,
                        )
                        if game_map.move_unit(
                            unit.unit_id,
                            self.state.original_unit_x,
                            self.state.original_unit_y,
                        ):
                            # Emit unit moved event for restoration
                            self._emit_unit_moved(
                                unit.name, unit.team, from_pos, to_pos
                            )
    ...
    def _auto_select_action_menu_item(self, unit) -> None:
        ...
        if not unit.can_act:
            ...
            if "Wait" in self.state.action_menu_items:
                self.state.action_menu_selection = self.state.action_menu_items.index(
                    "Wait"
                )
            return
        ...
        if cursor_pos in self.state.attack_range:
            ...
            if self.state.selected_unit_id:
                ...
                if unit and hasattr(unit.combat, "aoe_pattern"):
                    ...
                    self.state.aoe_tiles = game_map.calculate_aoe_tiles(
    ...
    def simple_enemy_turn(self) -> None:
        ...
        self._emit_turn_ended(Team.ENEMY)
        ...
        self._emit_turn_started(Team.PLAYER)
    ...
    def build_render_context(self) -> RenderContext:
        ...
        context.texts.append(TextRenderData(x=0, y=screen_height - 1, text=status_text))
    ...
    def end_player_turn(self) -> None:
        ...
        if self.state.current_team != old_team:
            ...
            self._emit_turn_ended(old_team_enum)
            self._emit_turn_started(new_team_enum)
    ...
    def _build_minimap_overlay(self) -> "OverlayRenderData":
        ...
        for map_y in range(0, game_map.height, scale_y):
            ...
            for map_x in range(0, game_map.width, scale_x * 2):
                ...
                camera_right = (
                    self.state.camera_x + self.renderer.get_screen_size()[0] - 28
                )  # Account for sidebar
                camera_top = self.state.camera_y
                camera_bottom = (
                    self.state.camera_y + self.renderer.get_screen_size()[1] - 3
                )  # Account for status
    ...
    def _get_minimap_char(
        self, game_map, unit_map: dict, x: int, y: int, scale_x: int, scale_y: int
    ) -> str:
    ...
    def update_banner_timing(self) -> None:
        ...
        if self.active_banner:
            ...
            duration = getattr(
                self, "banner_duration_ms", 2000
            )  # Default to 2000ms if not set
    ...
    def _emit_event(self, event) -> None:
        ...
        Args:
            event: GameEvent to emit to objectives
        """
    ...
    def _emit_unit_defeated(
        self, unit_name: str, team: Team, position: tuple[int, int]
    ) -> None:
        """Emit a unit defeated event."""
        event = UnitDefeated(
            turn=self.state.current_turn,
            unit_name=unit_name,
            team=team,
    ...
    def _emit_unit_spawned(
        self, unit_name: str, team: Team, position: tuple[int, int]
    ) -> None:
        """Emit a unit spawned event."""
        event = UnitSpawned(
            turn=self.state.current_turn,
            unit_name=unit_name,
            team=team,
    ...
    def _emit_turn_started(self, team: Team) -> None:
        """Emit a turn started event."""
        event = TurnStarted(turn=self.state.current_turn, team=team)
        self._emit_event(event)

    def _emit_turn_ended(self, team: Team) -> None:
